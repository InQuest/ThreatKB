...
def batch_update_c2dns():
    ...
    if batch:
        ...
        for b in batch['ids']:
            entity = c2dns.C2dns.query.get(b)
            if not entity:
                abort(404)
            if not current_user.admin and entity.owner_user_id != current_user.id:
                abort(403)
        ...
        if fields_to_update:
            db.session.execute(c2dns.C2dns.__table__.update().values(
                fields_to_update
            ).where(c2dns.C2dns.id.in_(batch['ids'])))
            db.session.commit()

        if batch['tags']:
            batch_create_tags_mapping(c2dns.C2dns.__tablename__, batch['ids'], batch['tags'])
